...
def render_scan(findings: list[RepoFinding], skipped_urls: list[str]) -> str:
    ...
    for finding in findings:
        ...
        lines.append("")
    lines.append("## Vertical Slice And Behavior Contract Prompts")
    lines.append("")
    lines.append("Use the repo evidence above to ask the user which end-to-end capabilities matter.")
    lines.append("For each accepted slice, capture intent, surfaces, dependencies, terms, merge order, and 1-3 behavior contracts.")
    lines.append("For each contract, ask for the initial state (Given), action or event (When), observable outcome (Then), and named evidence gate.")
    lines.append("")
    ...
    lines.append("- Vertical slices approved with stable behavior-contract IDs and evidence gates")
